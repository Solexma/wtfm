...
pub fn execute(cli: &Cli) {
    ...
    let answers = if config.is_none() {
        ...
        let config = crate::config::WtfmConfig {
            ...
            authors: answers.authors.iter().map(|author| {
                let parts: Vec<&str> = author.split('<').collect();
                let name = parts[0].trim().to_string();
                let email = parts[1].trim_matches(|c| c == '<' || c == '>' || c == ' ').to_string();
                Author { name, email }
            }).collect(),
        };
        ...
        WizardAnswers {
            ...
            authors: config.authors.iter().map(|a| format!("{} <{}>", a.name, a.email)).collect(),
        }
    };
    ...
    fs::write(readme_path.clone(), &readme_content)
